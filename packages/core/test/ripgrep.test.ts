...
/**
 * The first test to run acquires the ripgrep binary (download + extract), which
 * RipgrepBinary bounds with its own stage timeouts. Those budgets exceed the
 * suite's 30s default, so without this override a hung stage would still surface
 * as a generic test timeout naming nothing -- the exact dead end TKT-378 hit.
 */
const ACQUIRE_TIMEOUT = 120_000
