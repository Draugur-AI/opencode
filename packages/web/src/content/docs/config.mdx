...
{
  "$schema": "http://localhost:4096/config.json",
...
`$schema` points at your own running [server](/docs/server) — `4096` is the default port, but if it was already taken your instance picked a different one, or if the TUI assigned itself a random port. Use whatever `--port` you started it with, or pass `--port 4096` explicitly to make it predictable.
...
Your own running [server](/docs/server) publishes the server/runtime config schema at `/config.json`, and the TUI config schema at `/tui.json` — not `opencode.ai`, which describes upstream's shape rather than this fork's.
...
```json title="tui.json"
{
  "$schema": "http://localhost:4096/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
