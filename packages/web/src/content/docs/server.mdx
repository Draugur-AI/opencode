...
The server also publishes its own config and TUI config JSON Schemas, generated from this fork's actual config types:

```
http://<hostname>:<port>/config.json
http://<hostname>:<port>/tui.json
```

Point your editor's `$schema` at these (not `opencode.ai`'s) to validate against what this instance actually parses, e.g. `http://localhost:4096/config.json` in your `opencode.json`. Like every other route here, they sit behind [`OPENCODE_SERVER_PASSWORD`](#authentication) when it's set — an editor fetching `$schema` from a password-protected instance needs the same basic-auth credentials.
