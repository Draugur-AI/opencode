...
The server also publishes its own config and TUI theme JSON Schemas, generated from this fork's actual config types:
...
Point your editor's `$schema` at these (not `opencode.ai`'s) to validate against what this instance actually parses, e.g. `http://localhost:4096/config.json` in your `opencode.json`.
