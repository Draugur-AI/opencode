...
export const ProjectSettingsExtensions: Component = () => {
  ...
  const pluginRows = (items: string[]) => (
    <For each={items}>{(name) => <ExtensionRow icon="providers" name={name} />}</For>
  )

  const skillRows = (items: SkillItem[]) => (
    <For each={items}>{(item) => <ExtensionRow icon="brain" name={item.name} />}</For>
  )
