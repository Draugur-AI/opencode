...
// Shared by script/schema.ts (build-time CLI generation) and the server's /config.json and
// /tui.json routes (request-time generation) -- one generator, so the two can never drift out
// of sync with each other the way the astro-build artifact drifted out of sync with what
// actually gets embedded (TKT-394).
