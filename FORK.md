...
| Fourth `@opencode-ai/client-next` importer, `packages/app/src/utils/mcp-client.ts` (named `mcp-catalog-client.ts` at first, renamed the same day it widened), reaching the workspace client for **five** calls: `mcp.list`/`mcp.status` (the catalog + live status) and config-document's `targetRead`/`targetValidate`/`targetApply` (editing an MCP server through the same tab, TKT-323 MCP config editing) | Re-scoped twice, not assumed, both times: (1) the Integrations page cannot exist on either other client — the vendored tarball's `mcp.list` is a stale, differently-shaped `McpServer` type (a `pending` status this domain's union does not have) with no `status` method at all, and `@opencode-ai/sdk` v2 (`useServerSDK()`) only has the legacy `/mcp` surface, no `/api/mcp` or `/api/mcp/status`; (2) widening the SAME file (not a fifth) for config-document's three verbs, since MCP editing is the same feature this file already serves and needs them for the identical UI surface — a fifth file for the identical purpose would add nothing the bound exists to prevent. Scope explicitly bounded to what the MCP tab needs; skills/plugins/profiles editing gets its own widen-or-new decision when a second real consumer of config-document read/validate/apply exists, not before | n/a — fork-only packaging workaround | none | none | **remove** — deleted together with the vendored tarball and the three rows above by **TKT-328**. Bounded by construction, same discipline as the rows above: `mcp-client.ts` is this half's only importer and says so in its own header |
