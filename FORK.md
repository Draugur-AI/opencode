...
| `unit (linux)` — `tool.write`, "sets file permissions when writing sensitive data" | Not timer-flaky — deterministic and umask-dependent. Asserts a hardcoded `0o644`, which only holds under a `0022` umask; this runner's actual umask is `0002`, so the write comes out `0o664` every time. Confirmed pre-existing rather than PR-caused by a stronger check than the usual "3 reruns of one commit": reproduced identically on a completely clean, unmodified `dev` clone (TKT-322, feedback #194) — no relation to that PR's diff at all, so a single occurrence was enough to conclude it. | **Skipped** (TKT-322, feedback #194): `it.instance.skip` with a comment linking the feedback item. Un-skip condition: feedback #194 is triaged — compare against `(0o666 & ~process.umask())` instead of a hardcoded mode, or set the mode explicitly. |
