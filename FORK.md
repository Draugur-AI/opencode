...
| Fourth `@opencode-ai/client-next` importer, `packages/app/src/utils/mcp-client.ts` (named `mcp-catalog-client.ts` at first, renamed the same day it widened), reaching the workspace client for **six** calls: `mcp.list`/`mcp.status` (the catalog + live status) and config-document's `targetList`/`targetRead`/`targetValidate`/`targetApply` (adding, editing, and removing an MCP server through the same tab, TKT-323 MCP config editing — `targetList` resolves a target for a brand-new server with no existing catalog entry to read one off of) | Re-scoped twice, not assumed, both times: (1) the Integrations page cannot exist on either other client — the vendored tarball's `mcp.list` is a stale, differently-shaped `McpServer` type (a `pending` status this domain's union does not have) with no `status` method at all, and `@opencode-ai/sdk` v2 (`useServerSDK()`) only has the legacy `/mcp` surface, no `/api/mcp` or `/api/mcp/status`; (2) widening the SAME file (not a fifth) for config-document's four verbs, since MCP editing is the same feature this file already serves and needs them for the identical UI surface — a fifth file for the identical purpose would add nothing the bound exists to prevent. Scope explicitly bounded to what the MCP tab needs; skills/plugins/profiles editing gets its own widen-or-new decision when a second real consumer of config-document read/validate/apply exists, not before | n/a — fork-only packaging workaround | none | none | **remove** — deleted together with the vendored tarball and the three rows above by **TKT-328**. Bounded by construction, same discipline as the rows above: `mcp-client.ts` is this half's only importer and says so in its own header |
| Fifth `@opencode-ai/client-next` importer, `packages/app/src/utils/skill-client.ts`, reaching the workspace client for **two** calls: `skill.list` (the effective, winners-only view) and `skill.catalog` (every entry — winner and shadowed loser, with provenance and an enclosing config-document target, TKT-323 SkillCatalog) | Ruled, not a mis-widen: `mcp-client.ts` is scoped to the MCP tab by its own header, which explicitly says skills/plugins/profiles editing is not its job — widening it for an unrelated tab would violate that. A genuinely different, unrelated consumer needing verbs neither `@opencode-ai/client`'s vendored tarball (predates `skill.catalog` entirely) nor `@opencode-ai/sdk` v2 (`/api/skill/catalog` does not exist there — a new endpoint, and regenerating/publishing v2 SDK is its own release cycle) can serve, same shape as the MCP row two tickets ago. TKT-328's own sequence requires pipeline-governance fixes (dedup predicate, gen-split + CI drift guard) before any migration can safely happen — none of which exist yet — so riding a fifth importer now rather than waiting on that migration is the correct order, not a shortcut around it: the ledger is the migrations checklist this registers debt onto | n/a — fork-only packaging workaround | none | none | **remove** — deleted together with the vendored tarball and the four rows above by **TKT-328**. Bounded by construction, same discipline as the rows above: `skill-client.ts` is this half's only importer and says so in its own header |
...
| CI `typecheck` job: `package.json`'s `typecheck` script, `bun turbo typecheck` → `bun turbo typecheck --concurrency=2` | War-footing fix (Sean-directed, Ethan): the self-hosted runner OOM'd — 9 concurrent `tsgo --noEmit` at ~1.7GiB each (15.6GiB, 28.6% of host resident memory), turbo's default concurrency (10, uncapped by task or CLI flag) fanning out unbounded across all 32 typecheck-bearing packages with no interdependencies gating them. Measured locally (`ps -C tsgo`, exact-comm match, not a broader cmdline grep that double-counts wrapper processes): uncapped peak was ~23 concurrent processes / ~14GB RSS; `--concurrency=2` holds peak to exactly 2 concurrent `tsgo` processes / ~5.6GB, inside the ~4-6GiB target. Wall-clock cost: ~18s → ~20s locally on a fresh (non-cached) run — a small, worthwhile trade for not killing the shared host | Not a code bug, a resource-scheduling gap between the monorepo's package count and the runner's memory budget | n/a — fork-infra CI config only | none | none | **keep** — this runner's memory budget is fixed and shared with other fleets' concurrent jobs (see "CI runner reality" below); revisit only if the runner's own memory budget changes or `tsgo`'s per-process footprint drops |

| V1 bridge for the V2-only builtin tools: `packages/opencode/src/tool/{monitor,goal,ledger,history}.ts` (new) register `monitor_create`/`monitor_list`, `goal_get`/`goal_update_progress`, `ledger_add`, `history_search`/`history_get` into `packages/opencode/src/tool/registry.ts`'s hand-maintained `builtin` list, mirroring the existing `question`/`skill` wiring pattern — delegating to the same core services (`Monitor.Service`, `SessionGoal.Service`, `SessionLedger.Service`, `SessionHistorySearch`) the V2 tools already use, no duplicated logic | Sean-directed war-footing fix (relayed live, not filed as feedback): asked the agent on his live instance whether it had a monitor tool and it did not. Root cause traced (not assumed): this fork has TWO independent tool registries and TWO independent session runners. `packages/core/src/tool/registry.ts`'s `ToolRegistry.materialize()` (composes `BuiltInTools.node`, which correctly includes `MonitorTool`/`GoalTool`/`LedgerTool`/`HistoryTool` since TKT-321/322/335/#10) is consumed ONLY by `packages/core/src/session/runner/llm.ts`'s `runTurn` — which has ZERO callers anywhere in the codebase, confirmed by a repo-wide grep. The actual live serving path (`packages/opencode/src/session/processor.ts` → `session/llm.ts` → `session/tools.ts:92`) calls a wholly separate, hand-maintained legacy registry (`packages/opencode/src/tool/registry.ts`) that predates all four V2 tool families and was never told about any of them. Not a permission/profile deny (`profile-builtin.ts`'s `monitorRules` gates runtime execution, not registry visibility), not a stale-session cache artifact — confirmed with a genuinely fresh session against the live instance (`ses_008ad9011ffezt0Sw3ODRnucIB`, `172.16.14.0:4633`, sha `adec6000`) before writing any code | n/a — tool-wiring gap, not a proposal | none — no schema/table changes, delegates to existing core services | none | **Deliberately not (b)**: retiring the dual registry (wiring or deleting the dead `runTurn`, migrating `session/tools.ts` onto V2 `materialize()`) is filed as its own ticket per Ethan's ruling — it intersects the carry-ruling class and needs a design pass, not a war-footing patch. This row is the minimal unblock only: it adds tool VISIBILITY on the live path, it does not change which registry is authoritative long-term |

| `AppProcess.run()`'s timeout path (`packages/core/src/process.ts`) now succeeds with `timedOut: true` and whatever bytes were captured before the clock ran out, instead of discarding them and failing with a bare `AppProcessError`. `collectStream` takes an optional externally-visible `StreamAccumulator`, created before the racing `collect`/`Effect.timeoutOrElse` pair rather than inside it — mutation on that object survives fiber interruption (interruption stops further execution, it does not undo synchronous mutations already applied), which is what lets the timeout's `orElse` branch read partial bytes instead of nothing. `MonitorProcess.check` and `tool/bash.ts` (both consumers) updated to read `result.timedOut` instead of catching a "Timed out" `AppProcessError`; `monitor/runtime.ts`'s `runOne()` had its own SEPARATE discard (`rawOutput = result.type === "completed" ? result.output : ""`, hardcoding `""` for the timeout variant even though `CheckResult`'s own type already carried `output: string` on both variants) fixed alongside it — without that second fix, the Monitor consumer specifically would still have lost partial output one layer up from `MonitorProcess.check` (TKT-409, feedback #199) | Flagged during TKT-322 PR #38 review, filed as feedback #199 rather than fixed inline (a shared-`AppProcess`-behavior change was out of that slice's scope). The Monitor containment design (diary 2435 §3) wants a timed-out check's partial output bounded and retained, not silently dropped | not yet filed | none — no schema/table changes; `RunResult` gains an optional `timedOut` field, additive, no existing consumer signature broke (both real consumers updated in the same PR) | none | **fork-only** — `AppProcess` itself has no upstream equivalent (upstream's `BackgroundJob` explicitly disclaims restart survival and bounded capture; see the Command Monitor rows above for the fuller upstream-TODO context this whole subsystem sits under) |
